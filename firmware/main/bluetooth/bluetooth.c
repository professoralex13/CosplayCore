        ...
        // Detect button release
            ...
            if (current_state == 1) {
                press_duration = (xTaskGetTickCount() * portTICK_PERIOD_MS) -
                                 button_press_time;
                ESP_LOGI(TAG, "Button released, duration: %d ms",
                         press_duration);
                ...
                // Long press: could be used for other functions
                    ...
                    ESP_LOGI(TAG, "Long press detected - could trigger "
                                  "reset/clear pairings");
...
void bluetooth_init(i2s_chan_handle_t i2s_handle) {
    ...
    ESP_LOGI(TAG,
             "Bluetooth speaker ready. Press button to enter pairing mode.");
}
